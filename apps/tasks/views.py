...
def get_celery_all_tasks():
    ...
    for task in tasks:
        last_task = (
            TaskResult.objects.filter(task_name=task["script"])
        ...
        if last_task:
            ...
            task["successfull"] = (
                last_task.status == "SUCCESS" or last_task.status == "STARTED"
            )
...
def task_log(request):
    ...
    try:
        ...
        all_logs = [
            f
            for f in listdir(settings.CELERY_LOGS_DIR)
            if isfile(join(settings.CELERY_LOGS_DIR, f))
        ]
