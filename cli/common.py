...
def errInfo(aErrorCode):
    ...
    if COMMON.NA == aErrorCode:
        return "Not Set"
    if COMMON.ERR == aErrorCode:
        return "Error Generic"
    if COMMON.OK == aErrorCode:
        return "OK"
    if COMMON.NOT_FOUND == aErrorCode:
        return "Not Found"
    if COMMON.INPUT_ERR == aErrorCode:
        return "Input error"
...
def commonTxt(aCode):
    ...
    if COMMON.CFG_VAR_NA == aCode:
        return "CFG Var unknown typology"
    if COMMON.CFG_VAR_SIMPLE == aCode:
        return "CFG Var SIMPLE"
    if COMMON.CFG_VAR_LIST == aCode:
        return "CFG Var LIST"
    if COMMON.CFG_VAR_MIXED == aCode:
        return "CFG Var MIXT (list of dicts)"
...
def str_to_db_type(aStr):
    ...
    if "str" == aStr:
        return DbField.CHAR_FIELD
    if "string" == aStr:
        return DbField.CHAR_FIELD

    if "text" == aStr:
        return DbField.TEXT_FIELD

    if "float" == aStr:
        return DbField.FLOAT_FIELD

    if "date" == aStr:
        return DbField.DATE_FIELD
    if "time" == aStr:
        return DbField.DATE_FIELD

    if "bool" == aStr:
        return DbField.BOOL_FIELD
