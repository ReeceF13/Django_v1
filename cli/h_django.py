...
def add_model(aAppName, aModelName):
    ...
    model_code = file_load(
        os.path.join(DIR_ROOT, "templates", "generator", "model.tmpl")
    )
    if not model_code:
...
def add_model_field(aAppName, aModelName, aFieldName, aFieldType, **kwargs):
    ...
    if DbField.FK_FIELD == aFieldTypeDB:
        model_code_upd = add_fk_to_django_model(
            model_code,
            field_name=aFieldName,
            field_type=aFieldTypeDB,
            related_model=aFieldClass.__name__,
            on_delete="models.CASCADE",
            position=1,
            **kwargs,
        )
    else:
        model_code_upd = add_field_to_django_model(
            model_code,
            field_name=aFieldName,
            field_type=aFieldTypeDB,
            position=1,
